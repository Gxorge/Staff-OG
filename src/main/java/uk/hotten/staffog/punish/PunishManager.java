	...
	public void newPunishment(PunishEntry entry) {
		try (Connection connection = DatabaseManager.getInstance().createConnection()) {
			...
			if (entry.getPlayer() != null && entry.getPlayer().isOnline()) {
				...
				case MUTE -> {
					entry.getPlayer().sendMessage(Message.format("&cYou have been muted for &f" + entry.getReason()));
					entry.getPlayer().sendMessage(Message.format("&cYour mute " + (entry.getUntil() == -1 ? "does not expire." : "will expire in: \n"
							+ "&f" + duration)));
				}
				}
			}
		...
		try (Connection connection = DatabaseManager.getInstance().createConnection()) {
			...
			if (entry.getPlayer() != null && entry.getPlayer().isOnline()) {
				...
				String messageString = "&cYou have been kicked for:\n" +
						"&f" + entry.getReason() + "\n\n" +
						"&c" + "Please read our rules when you rejoin by running /rules";
			...
			Message.staffBroadcast(Message.formatNotification("KICK", entry.getName() + " has been kicked by " + entry.getByName()));
